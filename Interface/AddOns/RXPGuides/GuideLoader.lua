...
local embeddedGuideLoaderFrame = CreateFrame("Frame")
embeddedGuideLoaderFrame:Hide()
...
local function LoadEmbeddedGuide(n, guideData)
...
local function FinishEmbeddedGuidesLoading(callback)
    if addon.addonLoaded then
        embeddedGuides = nil
    else
        embeddedGuides = {}
...
local embeddedGuideLoadState
local function ProcessEmbeddedGuideBatch(self)
    local state = embeddedGuideLoadState
    if not state then
        self:Hide()
    ...
    local processed = 0
    while state.index <= #embeddedGuides and processed < state.batchSize do
        LoadEmbeddedGuide(state.index, embeddedGuides[state.index])
        state.index = state.index + 1
        processed = processed + 1
    end

    if state.index > #embeddedGuides then
        local callback = state.callback
        embeddedGuideLoadState = nil
        self:Hide()
        self:SetScript("OnUpdate", nil)
        FinishEmbeddedGuidesLoading(callback)
    end
end

local function StartEmbeddedGuidesLoading(callback)
...
function addon.LoadEmbeddedGuides()
    if not StartEmbeddedGuidesLoading() then return end

    for n, guideData in ipairs(embeddedGuides) do
        LoadEmbeddedGuide(n, guideData)
    end

    FinishEmbeddedGuidesLoading()
end

function addon.LoadEmbeddedGuidesAsync(callback)
    if not StartEmbeddedGuidesLoading(callback) then return end

    embeddedGuideLoadState = {
        index = 1,
        batchSize = 2,
        callback = callback,
    }
    embeddedGuideLoaderFrame:SetScript("OnUpdate", ProcessEmbeddedGuideBatch)
    embeddedGuideLoaderFrame:Show()
end
