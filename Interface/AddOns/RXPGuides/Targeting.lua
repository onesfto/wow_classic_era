...
local isEraClient = select(4, GetBuildInfo()) < 20000
...
function addon.targeting:Setup()
    ...
    if not isEraClient and addon.settings.profile.showTargetingOnProximity then
...
function addon.targeting.CheckTargetProximity()
    if isEraClient then return end
...
local TextBoxHook = function(self)
    if self.text and self.text.GetText and self.text:GetText() == actionForbiddenText then
...
function addon.targeting:UpdateTargetFrame(selector, OnUpdate)
    ...
    -- If proximity disabled, show all
    local friendlyList =
        (not isEraClient and addon.settings.profile.showTargetingOnProximity) and {} or targetList
    ...
    if (i == 0 and j == 0) or not addon.settings.profile.showEnabled then
        targetFrame:Hide()
    elseif not isEraClient and addon.settings.profile.showTargetingOnProximity then
