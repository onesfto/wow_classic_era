...
button={spellID=soulFire,option={isNative=true}},
handler=handler,
}
)
self:CreateEffect(
...
button={spellID=soulFire,option=false},
handler=handler,
}
)
...
[SAO.MOP]={spellID=incinerate,option={isNative=true}},
},
}
)
if SAO.IsMoP()then
...
button={spellID=incinerate,option=false},
}
)
