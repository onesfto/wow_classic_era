...
- **`chat`** — 暴雪内置聊天类型，左键用斜杠命令切输入框，右键操作 message group
- **`channel`** — 编号频道，左键先确保已加入再切过去，右键操作频道订阅
- **`tool`** — 功能按钮，行为写在 `onClick` 里

### 屏蔽标记（X 号）

贴图 `interface/common/voicechat-muted.blp`，与 !Pig 用的是同一张。

只有**右键是屏蔽功能**的按钮才会显示 X。所以「说」「队」（无右键）和「团」「战」（右键是发言）永远不显示。

刷新时机：右键点击后立即更新；登录后 3/5/10 秒各补刷一次（频道订阅状态要等聊天系统就绪才读得准，单次读取会拿到空值）。
...
位置：GW2_UI 设置窗口 → **附加组件 → 频道按钮**（与「一键换装」平级）。

由 `Options.lua` 的 `BuildPanel()` 构建，导出为 `addonTable.BuildChatBarPanel`，在 `core.lua` 里被塞进 `subPanels`。

选项构成：主开关 → 位置与外观（附着位置、X/Y 偏移、缩放、重置、渐隐、动态显隐）→ 频道屏蔽控制窗口 → 三组按钮显隐勾选（频道/功能/插件）。
...
> **前提：GW2_UI 设置里的「聊天表情」必须开着**，否则发出去就是 `:smile:` 字面量。
...
覆盖：文件加载、`Build()` 全流程、19 个按钮逐个左右键点击、鼠标进出、所有刷新路径、X 号屏蔽标记、隐藏后重排、动态显隐、未装插件不建按钮、设置面板构建与读写、`core.lua` 面板注册、诊断命令。
