-- GW2_UI_PLUS 任务日志皮肤 (QuestLog)
--
-- 把任务日志改成 714x487 的双栏宽面板，列表行数从 6 提到 22（不再依赖 Leatrix / !Pig），
-- 任务数量与折叠按钮回归暴雪原生排版，并彻底隐藏原生的追踪复选框。
--
-- 原先是直接改 GW2_UI/Games/Classic/Immersive/Skins/questLog.lua，现在整体搬进 PLUS。
--
-- 这里不需要自己驱动：上游的 evLoadSkins 会调用 GW.LoadQuestLogFrameSkin()，
-- 而它走的是 PLAYER_LOGIN 里的 C_Timer.After(0, ...)，比 PLUS 加载晚。
-- 下面 GW.LoadQuestLogFrameSkin = ... 覆盖掉上游那份，上游就会调到我们这一版——
-- 只跑一次，不会双跑。

local GW = _G.GW2_ADDON
if not GW then return end

-- 任务日志默认双栏尺寸，和专业面板保持同一套宽面板比例。
...
local function LoadQuestLogFrameSkin()
	...
	do
		...
		hooksecurefunc('QuestLog_Update', function()
			LayoutQuestLogFrame()
			QuestLogQuestCount:ClearAllPoints()
			-- 还原暴雪原生的排版布局：将任务数量 (任务: 8/20) 靠右对齐，Y 轴上移与“追踪任务”对齐
			QuestLogQuestCount:SetPoint('BOTTOMRIGHT', QuestLogListScrollFrame.backdrop, 'TOPRIGHT', 0, 9)
    ...
    QuestNPCModelTextFrame.tex:SetTexture("Interface/AddOns/GW2_UI/textures/party/manage-group-bg.png")
		...
		QuestLogCollapseAllButton:ClearAllPoints()
		-- 将“全部”展开/折叠按钮放置在第一个任务的正上方 (背景框体的左上角)，Y 轴上移与“追踪任务”对齐
		QuestLogCollapseAllButton:SetPoint("BOTTOMLEFT", QuestLogListScrollFrame.backdrop, "TOPLEFT", 2, 9)
