-- GW2_UI_PLUS 邮件皮肤 (Mail)
--
-- 上游的邮件皮肤只给正式服用：core/GW2_ui.lua 的 evLoadSkins 里
--     if not (GW.Classic or GW.TBC) then ... GW.LoadMailSkin() end
-- 经典旧世下从不调用。原来的做法是把那行挪出条件分支，再把这个文件改到经典旧世能跑。
--
-- 为什么必须整文件搬、不能从外部打补丁：
-- 下面第 4~7 行那几个 local MailFrame/InboxFrame/... 是文件级 upvalue，
-- 在文件解析时就绑定好了，外部插件永远够不着；而所有 skin 子函数也都是 local，
-- hooksecurefunc 无从下手。好在它对 GW2_UI 的依赖全是公开 API，搬过来即可。
--
-- 开关和位置记忆仍然用 GW2_UI 自己的 GW.settings（MAIL_SKIN_ENABLED / MAILBOX_POSITION），
-- 两边读写同一份存档。经典旧世下那个开关被上游 hidden 挡住了，PLUS 在
-- 「附加组件 → 界面皮肤」里另开了一个。
...
-- 上游在经典旧世不会调用 GW.LoadMailSkin，这里自己驱动一次，不存在双跑。
-- 邮件界面属于 FrameXML（经典旧世没有 Blizzard_MailUI 这个按需加载包），
-- PLAYER_LOGIN 时 MailFrame 一定已经存在。
local gwPlusMailLoader = CreateFrame("Frame")
gwPlusMailLoader:RegisterEvent("PLAYER_LOGIN")
gwPlusMailLoader:SetScript("OnEvent", function(self)
    self:UnregisterEvent("PLAYER_LOGIN")
    LoadMailSkin()
end)

if IsLoggedIn and IsLoggedIn() then
    gwPlusMailLoader:UnregisterEvent("PLAYER_LOGIN")
    LoadMailSkin()
end
