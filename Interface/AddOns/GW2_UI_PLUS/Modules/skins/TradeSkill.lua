-- GW2_UI_PLUS 专业/制造面板皮肤 (TradeSkill)
--
-- 把 Classic 的 TradeSkillFrame（专业）和 CraftFrame（制造/附魔）改造成 GW2 风格的
-- 双栏宽面板：GW2 标题头、可拖动、技能条/下拉/搜索框/数量输入全部重绘。
--
-- 这个模块本来就是自己写的（上游 GW2_UI 根本没有这个文件），原先塞在
-- GW2_UI/Games/Classic/Immersive/Skins/tradeskill.lua 里，现在整体搬进 PLUS。
-- 它只操作暴雪的全局框体、只 hook 全局函数，对 GW2_UI 的依赖仅限公开 API
-- （GW.CreateFrameHeaderWithBody / GW.SkinTextBox / GW.HandleNextPrevButton /
--   GW.UpdateCollapseTexture / GW.BackdropTemplates / GW.NoOp），所以搬家零成本。
--
-- 相对原版的改动只有两处：取 GW 的方式，以及开关从 GW.settings 换成 PLUS 自己的存档。
...
local function LoadTradeSkillFrameSkin()
    -- 开关改用 PLUS 自己的存档。GW2_UI 的 XXX_SKIN_ENABLED 是 defaults2.lua 里的
    -- 硬编码表，没有对外注册接口，外部插件挤不进去。默认开启（缺省即为 nil）。
...
-- 原先是 GW2_UI 的 core/GW2_ui.lua 里加一行来调用，现在自己驱动。
-- 必须等到 PLAYER_ENTERING_WORLD：存档变量要 ADDON_LOADED 之后才可用，
-- 而两个专业界面都是按需加载的，一定在这之后才会出现。
