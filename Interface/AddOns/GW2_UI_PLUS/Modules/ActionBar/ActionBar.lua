...
function AB.EnsureMoverSettings(settingName, default)
    ...
    -- AceDB 会把与默认值相同的 point/relativePoint 从存档中裁掉。因为这个
    -- 默认项是在 AceDB 初始化后动态补入的，重载时已有的残缺子表拿不到默认
    -- 元表；RegisterMovableFrame 便会判定坐标无效并回到默认位置。
...
-- 参照本体 HUD 缩放的算法，只更新血球、位移条和中央 HUD 背景各自的缩放倍数。
...
function AB.ApplyGlobeScale()
    ...
    -- 本体的位移条以 UIParent 为父级，只锚在血球上，因此必须独立同步缩放。
