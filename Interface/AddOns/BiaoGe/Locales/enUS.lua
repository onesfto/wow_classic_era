...
do
    ...
    L["|Hgarrmission:BiaoGeBattleNetRoles:%s|h[复制]|h"] = "|Hgarrmission:BiaoGeBattleNetRoles:%s|h[Copy]|h"
    L["该玩家一共%s个满级角色：|cffFFFFFF%s|r %s"] = "This player has %s max-level characters: |cffFFFFFF%s|r %s"
    L["正在查询战网角色"] = "Querying Battle.net characters"
    L["服务器连接失败，请尝试重载游戏"] = "Server connection failed. Try reloading the game."
    L["输入玩家名字"] = "Enter player name"
    L["查询"] = "Query"
    L["该战网%s个满级角色"] = "%s max-level characters on this Battle.net account"
    L["说明：数据来自官方接口"] = "Note: Data is provided by the official API"
    L["如果角色不存在或没满级，那么查询结果可能不准确。"] = "Results may be inaccurate if the character does not exist or is not max level."
    L["黑名单"] = "Blacklist"
    L["黑"] = "B"
    L["|cffff0000提醒：它已被你拉黑"] = "|cffff0000Warning: You have blacklisted this player"
    L["|cffbbbbbb（被拉黑的角色在聊天发言时会有红色[黑]标记）"] = "|cffbbbbbb (Blacklisted characters are marked with a red [B] in chat)"
    L["加入黑名单"] = "Add to Blacklist"
    L["取消黑名单"] = "Remove from Blacklist"
    L["添加"] = "Add"
    L["导出"] = "Export"
    L["导入"] = "Import"
    L["黑名单为空，无数据可导出。"] = "The blacklist is empty."
    L["导出黑名单"] = "Export Blacklist"
    L["导入黑名单"] = "Import Blacklist"
    L["将合并到现有黑名单，不会覆盖已有数据"] = "Imported entries will be merged with the current blacklist."
    L["确认导入"] = "Import"
