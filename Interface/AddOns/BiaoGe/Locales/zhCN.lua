...
do
    ...
    L["查询全部角色"] = true
    L["|Hgarrmission:BiaoGeBattleNetRoles:%s|h[复制]|h"] = true
    L["该玩家一共%s个满级角色：|cffFFFFFF%s|r %s"] = true
    L["正在查询战网角色"] = true
    L["服务器连接失败，请尝试重载游戏"] = true
    L["输入玩家名字"] = true
    L["查询"] = true
    L["该战网%s个满级角色"] = true
    L["说明：数据来自官方接口"] = true
    L["如果角色不存在或没满级，那么查询结果可能不准确。"] = true
    L["黑名单"] = true
    L["黑"] = true
    L["|cffff0000提醒：它已被你拉黑"] = true
    L["|cffbbbbbb（被拉黑的角色在聊天发言时会有红色[黑]标记）"] = true
    L["加入黑名单"] = true
    L["取消黑名单"] = true
    L["添加"] = true
    L["导出"] = true
