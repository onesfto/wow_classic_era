...
do --简体说明书
    ...
    ns.updateText_before = {
        "|cff00FF00" .. [[6月27日更新v2.0.4]],
        [[-自动拍卖：同时拍卖的最大数量从8个改为10个]],
        [[-团长拍卖面板：增加拍卖数量（比如相同套装可以一下拍3个）]],
        [[-交易退货：退货后现在会弹窗是否重拍此装备]],
        [[-拍卖记录：流拍页现在可以通过CTRL或SHIFT多选，对多件装备发起重拍（与未拍页的操作类似）]],
        [[-拍卖记录：未拍页和流拍页现在支持框选操作]],
        [[-角色总览设置：专业CD里增加多个选项"忽略某个专业CD"（默认不勾选）]],
        " ",
        "|cff00FF00" .. [[6月25日更新v2.0.3]],
        [[-<时光服>增加P5双本表格]],
        [[-<时光服/TBC>角色总览：增加"已完成日常数量"（默认不勾选）]],
        [[-角色总览："专业CD"现在合并为一项，不像以前每个专业单独分开，以节省显示空间]],
        [[-自动拍卖设置：增加选项"未关注且未加入心愿单的装备自动折叠"（默认不勾选）]],
        [[-第二代自动拍卖：团长端的"取消拍卖"和"暂停拍卖"按钮现在直接显示在面板上，而不是在更多操作里]],
        " ",
        "|cff00FF00" .. [[6月20日更新v2.0.2]],
        [[-第二代自动拍卖：修复了匿名模式卡秒出价时自动发消息的问题]],
        [[-修复了一个导致插件加载失败的问题]],
        " ",
        "|cff00FF00" .. [[6月19日更新v2.0.0]],
        c1 .. [[-新增第二代自动拍卖]],
        c1 .. [[·团长可在拍卖面板里选择使用第一代还是第二代]],
        c1 .. [[·第二代需要全团表格版本高于2.0.0，否则团员会看不到拍卖窗口]],
        c1 .. [[·第二代拍卖支持常规与匿名模式、暂停拍卖、相同物品联动刷新]],
        [[-拍卖记录：流拍装备的右键菜单中增加"向团长申请重拍"]],
        [[·点击按钮后，团长会收到弹窗"XXX申请重拍XXX装备，流拍价XXX"]],
        [[·团长点击确定后，会自动重拍该装备，拍卖时长为20秒]],
        [[-拍卖记录：团长自己买的装备，现在默认状态为已交易]],
        [[-装备过滤：增加选项"忽略战网绑定"（默认勾选。战网绑定物品即使被其他过滤规则命中，也不会被隐藏）]],
        [[-装备过滤：修复了猎人会错误地过滤双手斧和双手剑的问题]],
        [[-自动拍卖设置：增加选项"锁定拍卖竞价窗口"（默认不勾选）]],
        [[-角色总览设置：增加选项"使用黑白着色"（默认不勾选）]],
        [[-<时光服>角色总览：橙装升级物品现在会显示具体数量]],
        [[-<正式服>表格：增加新BOSS"腐沼"]],
    }
...
do
    ...
    L["由于你的部分角色同时存在于多个子战网，导致同步异常。输入该命令查看解决办法：/bgre"] = true
...
do
    ...
    L['%s 向你申请重拍流拍装备：\n\n%s%s（流拍价：%s）\n\n是否重拍该装备？'] = true
    L['向团长申请重拍'] = true
    L['团长的BiaoGe版本高于v2.0.0时才能收到你的请求。'] = true
