...
-- Safe wrapper: always returns a numeric rank (nil → 0)
local function CSC_GetTalentRank(tabIndex, talentIndex)
    local rank = select(5, CSC_GetTalentInfo(tabIndex, talentIndex))
    if rank == nil then
        return 0
    end
    return rank
...
function CSC_GetMageCritStatsFromTalents()
    ...
    local spellRank = CSC_GetTalentRank(1, 15)
    ...
    spellRank = CSC_GetTalentRank(2, 13)
    if (spellRank > 0) and (spellRank <= 3) then
...
function CSC_GetMageSpellHitFromTalents()
    ...
    -- Arcane Focus
    local spellRank = CSC_GetTalentRank(1, 2)
    arcaneHit = spellRank * 2 -- 2% for each point

    -- Elemental Precision
    spellRank = CSC_GetTalentRank(3, 3)
...
function CSC_GetWarlockSpellHitFromTalents()
    ...
    -- Suppression
    local spellRank = CSC_GetTalentRank(1, 1)
...
function CSC_GetWarlockCritStatsFromTalents()
    ...
    local devastationCrit = CSC_GetTalentRank(3, 7)
...
function CSC_GetPriestBonusHealingModifierFromTalents()
    -- Spiritual Healing
    local spellRank = CSC_GetTalentRank(2, 15)
...
function CSC_GetPaladinCritStatsFromTalents()
    ...
    local spellRank = CSC_GetTalentRank(1, 13)
...
local function CSC_GetPaladinDefenseFromTalents()
    ...
    local spellRank = CSC_GetTalentRank(2, 9)
    if (spellRank > 0) and (spellRank <= 5) then
...
function CSC_GetPaladinImprovedBoWModifier()
    ...
    local spellRank = CSC_GetTalentRank(1, 10)
...
local function CSC_GetWarriorDefenseFromTalents()
    ...
    -- Anticipation (2, 4, 6, 8, 10)%
    local spellRank = CSC_GetTalentRank(3, 2)
    if (spellRank > 0) and (spellRank <= 5) then
        defense = defenseTable[spellRank]
    end
...
function CSC_GetShamanHitFromTalents()
    -- Nature's Guidance
    local spellRank = CSC_GetTalentRank(3, 6)
    return spellRank
end
...
function CSC_GetShamanCallOfThunderCrit()
    ...
    local spellRank = CSC_GetTalentRank(1, 8)
...
function CSC_GetShamanTidalMasteryCrit()
    ...
    local spellRank = CSC_GetTalentRank(3, 11)
    return spellRank
end
...
function CSC_GetMP5ModifierFromTalents(unit)
    ...
    if unitClassId == CSC_PRIEST_CLASS_ID then
        -- Meditation
        spellRank = CSC_GetTalentRank(1, 8)
    elseif unitClassId == CSC_MAGE_CLASS_ID then
        -- Arcane Meditation
        spellRank = CSC_GetTalentRank(1, 12)
    elseif unitClassId == CSC_DRUID_CLASS_ID then
        -- Reflection
        spellRank = CSC_GetTalentRank(3, 6)
    end
