--[[
    ...
    Decursive (v 2.8.0) add-on for World of Warcraft UI
...
local function InitVariables_Once() -- {{{
    ...
    D.Revision = "ab6dc87"; -- not used here but some other add-on may request it from outside
    D.date = "2026-07-05T15:15:08Z";
    D.version = "2.8.0";

    if D.date ~= "@project".."-date-iso@" then
        -- 1783264508 doesn't work
...
function D:VersionWarnings(forceDisplay) -- {{{
    ...
    if DcrMaxTOC < T._tocversion then
        ...
        elseif time() - self.db.global.TocExpiredDetection > 3600 * 24 * 40 or debug then -- if more than 40 days elapsed since the detection
            ...
            if time() - self.db.global.LastExpirationAlert > 48 * 3600 or forceDisplay or debug then
                ...
                T._ShowNotice ("|cff00ff00Decursive version: 2.8.0|r\n\n" .. "|cFFFFAA66" .. L["TOC_VERSION_EXPIRED"] .. "|r");
    ...
    if (("2.8.0"):lower()):find("beta") or ("2.8.0"):find("RC") or ("2.8.0"):find("Candidate") or alpha then
        ...
        if self.db.global.NonRelease ~= "2.8.0" then
            self.db.global.NonRelease = "2.8.0";
            T._ShowNotice ("|cff00ff00Decursive version: 2.8.0|r\n\n" .. "|cFFFFAA66" .. (("2.8.0"):find("RC") and L["ER_VERSION_NOTICE"] or L["DEV_VERSION_ALERT"]) .. "|r");
        end
    end
...
T._LoadedFiles["DCR_init.lua"] = "2.8.0";
...
ab6dc875d0335794fea5b03a7117526aa7320a4c
