--[[
    ...
    This file was last updated on 2026-08-17T20:50:41Z
--]]
-------------------------------------------------------------------------------
...
local function InitVariables_Once() -- {{{
    ...
    D.Revision = "d763896"; -- not used here but some other add-on may request it from outside
    D.date = "2026-08-21T18:04:56Z";
    D.version = "2.8.3";

    if D.date ~= "@project".."-date-iso@" then
        -- 1787335496 doesn't work
...
function D:VersionWarnings(forceDisplay) -- {{{
    ...
    if DcrMaxTOC < T._tocversion then
        ...
        elseif time() - self.db.global.TocExpiredDetection > 3600 * 24 * 40 or debug then -- if more than 40 days elapsed since the detection
            ...
            if time() - self.db.global.LastExpirationAlert > 48 * 3600 or forceDisplay or debug then
                ...
                T._ShowNotice ("|cff00ff00Decursive version: 2.8.3|r\n\n" .. "|cFFFFAA66" .. L["TOC_VERSION_EXPIRED"] .. "|r");
    ...
    if (("2.8.3"):lower()):find("beta") or ("2.8.3"):find("RC") or ("2.8.3"):find("Candidate") or alpha then
        ...
        if self.db.global.NonRelease ~= "2.8.3" then
            self.db.global.NonRelease = "2.8.3";
            T._ShowNotice ("|cff00ff00Decursive version: 2.8.3|r\n\n" .. "|cFFFFAA66" .. (("2.8.3"):find("RC") and L["ER_VERSION_NOTICE"] or L["DEV_VERSION_ALERT"]) .. "|r");
        end
    end
...
T._LoadedFiles["DCR_init.lua"] = "2.8.3";
...
d7638969543fed66d5116a3ea0a93f8be15b1eff
