local onesftoConfig = {
    ...
    -- 浮动窗口位置与其它界面配置示例：
    -- 如果你在游戏中移动了 NWB 窗口，可以在 WTF 文件中找到下面的字段并提取到此处
    -- ["timerWindowX"] = 0,
    -- ["timerWindowY"] = 0,
    -- ["layerWindowX"] = 0,
    -- ["layerWindowY"] = 0,
...
local function ApplyNWBConfig()
    if NWBdatabase and NWBdatabase["global"] then
        for k, v in pairs(onesftoConfig) do
            NWBdatabase["global"][k] = v
        end
    end
end
...
frame:SetScript("OnEvent", function(self, event, ...)
    if event == "ADDON_LOADED" and ... == "NovaWorldBuffs" then
        ApplyNWBConfig()
    elseif event == "PLAYER_LOGIN" then
        if IsAddOnLoaded("NovaWorldBuffs") then ApplyNWBConfig() end
                ...
                if E and E.GetModule then
                    ...
                    if S then
                        ...
                        if MinimapLayerFrame then
                            ...
                            MinimapLayerFrame:SetPoint("TOPRIGHT", UIParent, "TOPRIGHT", -63, -20)
