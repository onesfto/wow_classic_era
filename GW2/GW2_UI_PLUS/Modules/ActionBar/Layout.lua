...
-- GW2_UI 只在首次创建状态层时设置尺寸，重排后需同步所有状态层。
local function SyncPetButtonStateLayerSize(button)
    if not button or not button.GetWidth then return end
    local size = button:GetWidth()
    local interruptDisplay = button.InterruptDisplay
    local base = interruptDisplay and interruptDisplay.Base
        and interruptDisplay.Base.Base
    if base and base.SetSize then base:SetSize(size, size) end
    local mask = button.gwInterruptDisplayMask
    if mask and mask.SetSize then mask:SetSize(size, size) end
    local shine = button.Shine
    if shine and shine.SetSize then shine:SetSize(size, size) end
    local flash = button.Flash
    if flash and flash.SetSize then flash:SetSize(size, size) end
end
...
function Layout.ApplyPetBar()
    ...
    for index, button in ipairs(petFrame.buttons) do
        ...
        if visible then
            ...
            SyncPetButtonStateLayerSize(button)
