...
ApplyMoverDefault = function(GW, moverInfo, default, force)
    ...
    if IsPlusProfileDefault()
        and (moverInfo.setting == "MultiBarBottomLeft"
            or moverInfo.setting == "MultiBarBottomRight") then
        -- These Plus positions differ from the native mainbar layout defaults.
        saved.hasMoved = true
    end
    ...
    frame.isMoved = saved.hasMoved == true
    frame:SetAttribute("isMoved", frame.isMoved)
