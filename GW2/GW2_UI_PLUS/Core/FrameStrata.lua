...
local ACTION_BAR_CHILD_LEVEL = NORMAL_FRAME_LEVEL + 1
...
local function ApplyActionBars()
    ...
    for _, name in ipairs(ACTION_BAR_NAMES) do
        local frame = _G[name]
        if frame then
            ApplyDirectChildren(frame, ACTION_BAR_CHILD_LEVEL)
        end
    end
    ...
    if mageBar and type(mageBar.GetChildren) == "function" then
        for _, button in ipairs({mageBar:GetChildren()}) do
            ApplyFrame(button, ACTION_BAR_CHILD_LEVEL)
            ApplyDirectChildren(button, ACTION_BAR_CHILD_LEVEL)
        end
    end

    ApplyIndexedFrames("GwPlusMageBarButton", 1, 24, ACTION_BAR_CHILD_LEVEL)
    for index = 1, 24 do
        ApplyDirectChildren(_G["GwPlusMageBarButton" .. index], ACTION_BAR_CHILD_LEVEL)
