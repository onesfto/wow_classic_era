...
local function NewLayoutNode()
    ...
    function node:ClearAllPoints() self.point = nil end
...
assert(db.portraitOffsetX == 0 and db.portraitOffsetY == 0,
    "宠物头像默认偏移错误")
assert(db.auraPosition == "TOPRIGHT", "宠物光环默认位置错误")
assert(db.auraOffsetX == 0 and db.auraOffsetY == 0,
    "宠物光环默认偏移错误")
...
petMain.auras = NewLayoutNode()
function petMain.auras:ForceUpdate()
    self.forceUpdateCalls = (self.forceUpdateCalls or 0) + 1
end
...
assert(petMain.auras.point[1] == "TOPRIGHT"
        and petMain.auras.point[2] == petMain
        and petMain.auras.point[3] == "TOPRIGHT"
        and petMain.auras.point[4] == 0
        and petMain.auras.point[5] == 0
        and not petMain.auraPositionUnder
        and petMain.auras.forceUpdateCalls == 1,
    "宠物光环默认位置未应用")
db.auraPosition = "BOTTOMLEFT"
db.auraOffsetX = 12
db.auraOffsetY = 9
assert(petFrame.ApplyLayout(), "宠物光环位置更新失败")
assert(petMain.auras.point[2] == petMain
        and petMain.auras.point[3] == "BOTTOMLEFT"
        and petMain.auras.point[4] == 12
        and petMain.auras.point[5] == 9
        and petMain.auraPositionUnder,
    "宠物光环位置或偏移未按设置应用")
db.healthWidth = 230
...
local auraOptionsByName = {}
for _, option in ipairs(framePanel.gwOptions) do
    auraOptionsByName[option.optionName or option.name] = option
end
assert(not auraOptionsByName.PET_AURAS_UNDER,
    "宠物光环设置不应保留旧的下方显示开关")
local auraPositionOption = auraOptionsByName.GW2PlusPetAuraPosition
local auraOffsetXOption = auraOptionsByName.GW2PlusPetAuraOffsetX
local auraOffsetYOption = auraOptionsByName.GW2PlusPetAuraOffsetY
assert(auraPositionOption
        and table.concat(auraPositionOption.optionsList, ",")
            == "TOPLEFT,TOPRIGHT,BOTTOMLEFT,BOTTOMRIGHT"
        and table.concat(auraPositionOption.optionNames, ",")
            == "左上,右上,左下,右下"
        and auraPositionOption.groupHeaderName == "光环",
    "宠物光环位置选项错误")
assert(auraOffsetXOption and auraOffsetYOption
        and auraOffsetXOption.min == -300
        and auraOffsetXOption.max == 300
        and auraOffsetYOption.min == -100
        and auraOffsetYOption.max == 100
        and auraOffsetXOption.name == "X 偏移"
        and auraOffsetYOption.name == "Y 偏移"
        and auraOffsetXOption.gwPlusColumns == 2
        and auraOffsetYOption.gwPlusColumns == 2,
    "宠物光环偏移选项错误")
...
assert(db.portraitEnabled == true and db.portraitSize == 60
        ...
        and db.auraPosition == "TOPRIGHT"
        and db.auraOffsetX == 0
        and db.auraOffsetY == 0,
    "宠物头像设置恢复默认失败")
for _, option in ipairs(framePanel.gwOptions) do
