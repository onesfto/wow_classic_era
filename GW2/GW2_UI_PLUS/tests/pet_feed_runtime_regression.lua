...
assert(db.buttonSize == 25 and db.useLowLevelFirst
    ...
    "宠物喂食默认值错误")
db.buttonSize = 64
_G.GwPlusPetFeed:SetSize(64, 64)
...
assert(_G.GwPlusPetFeed.attributes.type1 == "spell"
    ...
    "喂食按钮安全法术属性错误")
assert(_G.GwPlusPetFeed.Border
        and _G.GwPlusPetFeed.Border.texture
            == "Interface/AddOns/GW2_UI/textures/bag/bagitemborder.png"
        and _G.GwPlusPetFeed.Border.width == 64
        and _G.GwPlusPetFeed.Border.height == 64
        and _G.GwPlusPetFeed.Border.allPoints == _G.GwPlusPetFeed,
    "喂食按钮应使用与动作条技能相同尺寸和位置的金色边框")
assert(not _G.GwPlusPetFeed.Border:IsShown(),
    "鼠标未悬停时喂食按钮不应显示边框")
_G.GwPlusPetFeed.scripts.OnEnter(_G.GwPlusPetFeed)
assert(_G.GwPlusPetFeed.Border:IsShown(),
    "鼠标移入喂食按钮时应显示边框")
_G.GwPlusPetFeed.scripts.OnLeave(_G.GwPlusPetFeed)
assert(not _G.GwPlusPetFeed.Border:IsShown(),
    "鼠标移出喂食按钮时应隐藏边框")
assert(_G.GwPlusPetFeed.icon.point
        and _G.GwPlusPetFeed.icon.point[1] == "CENTER"
        and _G.GwPlusPetFeed.icon.point[3] == "CENTER"
        and _G.GwPlusPetFeed.icon.point[4] == 0
        and _G.GwPlusPetFeed.icon.point[5] == 0,
    "喂食图标不能相对金边向上偏移")
