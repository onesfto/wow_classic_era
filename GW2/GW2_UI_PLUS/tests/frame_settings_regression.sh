...
if grep -F 'PET_AURAS_UNDER' "$settings_file" >/dev/null; then
    echo "宠物设置不应保留旧的下方显示光环选项" >&2
    exit 1
fi
grep -F 'GW2PlusPetAuraPosition' "Modules/UnitFrames/PetFrame.lua" >/dev/null
grep -F 'GW2PlusPetAuraOffsetX' "Modules/UnitFrames/PetFrame.lua" >/dev/null
grep -F 'GW2PlusPetAuraOffsetY' "Modules/UnitFrames/PetFrame.lua" >/dev/null
