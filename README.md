# World of Warcraft Classic Era Addons

当前安装的插件及作用说明（按功能分类）：

## 核心/界面增强 (UI & Core)
- **ElvUI** (及 _Libraries, _Options): 全面替换原生界面的高度自定义 UI 框架。
- **!Pig** (及 _RurutiaAudio): PIG 综合插件包，提供金团管理、功能增强等。
- **AdvancedInterfaceOptions**: 恢复被暴雪隐藏的系统高级设置选项。
- **BlizzMove** (及 _Debug): 允许自由移动系统默认的各个界面窗口。
- **OmniCC** (及 _Config): 技能冷却倒计时（在图标上显示数字）。
- **Plater**: 强大的姓名板（血条）增强插件。
- **TargetNameplateIndicator**: 目标姓名板指示器（高亮当前目标）。
...
- **DBM (Deadly Boss Mods)** 系列 (及 _VPRurutia 等): 核心团队副本与地下城首领报警插件，提供倒计时和技能提示。
- **Details** 系列 (及 _TinyThreat, _Vanguard 等): 强大的伤害/治疗统计插件，包含仇恨、对战比较等扩展。
- **WeakAuras** (及各子模块): 神级插件，允许自定义几乎所有的战斗监视、增益/减益提示。
- **ThreatClassic2**: 经典旧世仇恨值统计仪。
- **Decursive**: 一键驱散/解诅咒助手。
...
- **Auctionator**: 拍卖行增强，提供快速购买、出售和历史价格记录。
- **BiaoGe** (及 _Rurutia): 金团表格与账本管理工具。
...
## 邮件与社交 (Mail & Social)
- **Postal**: 邮件增强，支持一键收取、发送。
- **InboxMailBag**: 将邮箱界面变为背包模式。
- **MailLogger**: 邮件收发记录。
...
- **CopyAnything**: 允许复制游戏内的大多数文本。
- **GFW_FeedOMatic**: 猎人一键喂宠。
...
- **MRT (Method Raid Tools)**: 团队战术板及检查工具。

*(注：带 _Options、_Data、_Debug 等后缀的条目均为对应主插件的设置或数据依赖模块。)*
