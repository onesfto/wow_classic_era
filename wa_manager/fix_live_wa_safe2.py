...
def repl_start(m): return new_start
def repl_finish(m): return new_finish
def repl_init(m): return new_init

def replacer(match):
    block = match.group(0)
    b = re.sub(r'\["init"\]\s*=\s*\{.*?\n\},', repl_init, block, flags=re.DOTALL)
    b = re.sub(r'\["start"\]\s*=\s*\{.*?\n\},', repl_start, b, flags=re.DOTALL)
    b = re.sub(r'\["finish"\]\s*=\s*\{.*?\n\},', repl_finish, b, flags=re.DOTALL)
    return b
